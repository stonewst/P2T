...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # 🔥 TTRL (SR-TTRL): allow the caller to override sampling params for the
        # auxiliary reflection passes (summarization / verification) through meta_info.
        # The max_tokens override is required because reflection prompts can be much
        # longer than a normal training prompt while their outputs are short, so
        # reserving the full response_length for them would overflow the model context
        # window. Temperature / top_p / top_k allow do_sample decoding for reflection.
        # These overrides only take effect when the keys are present, so normal
        # training / validation rollouts are unaffected.
        reflect_max_tokens = prompts.meta_info.get("reflect_max_tokens", None)
        if reflect_max_tokens is not None:
            kwargs["max_tokens"] = int(reflect_max_tokens)
            kwargs["n"] = 1
            if prompts.meta_info.get("reflect_temperature", None) is not None:
                kwargs["temperature"] = prompts.meta_info["reflect_temperature"]
            if prompts.meta_info.get("reflect_top_p", None) is not None:
                kwargs["top_p"] = prompts.meta_info["reflect_top_p"]
            if prompts.meta_info.get("reflect_top_k", None) is not None:
                kwargs["top_k"] = prompts.meta_info["reflect_top_k"]
