...
def compute_grpo_outcome_advantage(
    ...
    Args:
        ...
        config: `(Optional[AlgoConfig])`
            algorithm configuration object, which may contain:
            - adv_all_wrong: advantage value when all responses in a group are wrong
            - adv_all_correct: advantage value when all responses in a group are correct

    Note:
        If norm_adv_by_std_in_grpo is True, the advantage is scaled by the std, as in the original GRPO.
        If False, the advantage is not scaled, as in Dr.GRPO (https://arxiv.org/abs/2503.20783).
        Special cases:
        - If all responses in a group are correct, each response's advantage is set to adv_all_correct
        - If all responses in a group are wrong, each response's advantage is set to adv_all_wrong
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    # Get special case advantage values and enable flags from config
    assert config is not None
    enable_customed_adv_for_all_correct = config.enable_customed_adv_for_all_correct
    enable_customed_adv_for_all_wrong = config.enable_customed_adv_for_all_wrong
    adv_all_wrong = config.adv_all_wrong
    adv_all_correct = config.adv_all_correct
    ...
    id2mean = {}
    id2std = {}
    if enable_customed_adv_for_all_correct:
        id2all_correct = {}
    if enable_customed_adv_for_all_wrong:
        id2all_wrong = {}
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                if id2score[idx][0] == reward_wrong and reward_wrong == 0:
                    id2mean[idx] = torch.tensor(-1.0)
                if enable_customed_adv_for_all_correct:
                    id2all_correct[idx] = False
                if enable_customed_adv_for_all_wrong:
                    id2all_wrong[idx] = False
            elif len(id2score[idx]) > 1:
                scores_tensor = torch.stack(id2score[idx])
                id2mean[idx] = torch.mean(scores_tensor)
                id2std[idx] = torch.std(scores_tensor)
                
                # Check if all responses are correct (reward = reward_correct) or all are wrong (reward = reward_wrong)
                # Assuming reward > 0 means correct, reward <= 0 means wrong
                if enable_customed_adv_for_all_correct:
                    all_correct = torch.all(scores_tensor == reward_correct).item()
                    id2all_correct[idx] = all_correct
                if enable_customed_adv_for_all_wrong:
                    all_wrong = torch.all(scores_tensor == reward_wrong).item()
                    id2all_wrong[idx] = all_wrong
            else:
                raise ValueError(f"no score in prompt index: {idx}")
        
        for i in range(bsz):
            idx = index[i]
            # Handle special cases only if enabled in config
            if enable_customed_adv_for_all_correct and id2all_correct[idx]:
                scores[i] = torch.tensor(adv_all_correct, device=scores.device)
            elif enable_customed_adv_for_all_wrong and id2all_wrong[idx]:
                scores[i] = torch.tensor(adv_all_wrong, device=scores.device)
            else:
                # Normal GRPO advantage calculation
...
def _outcome_advantage(
    outcome_token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    config: "AlgoConfig",
    epsilon: float,
    ...
    """Outcome advantage under one of three normalization modes.

        norm_mode in (None, "null", "none") : raw outcome reward, broadcast over tokens
        norm_mode == "group-mean"           : subtract the group mean       (Dr.GRPO)
        norm_mode == "group-mean-std"       : subtract mean, divide by std  (GRPO)

    The two group-* modes reuse compute_grpo_outcome_advantage, so the top-level
    all-correct / all-wrong special cases (config.enable_customed_adv_for_all_*)
    still apply. The raw mode does no grouping at all.
    """
    if norm_mode in (None, "null", "none", "None"):
        scores = outcome_token_level_rewards.sum(dim=-1)              # (bs,)
        return scores.unsqueeze(-1) * response_mask

    if norm_mode == "group-mean":
        a, _ = compute_grpo_outcome_advantage(
            token_level_rewards=outcome_token_level_rewards, response_mask=response_mask,
            index=index, epsilon=epsilon, norm_adv_by_std_in_grpo=False, config=config,
        )
        return a
    if norm_mode == "group-mean-std":
        a, _ = compute_grpo_outcome_advantage(
            token_level_rewards=outcome_token_level_rewards, response_mask=response_mask,
            index=index, epsilon=epsilon, norm_adv_by_std_in_grpo=True, config=config,
        )
        return a
    raise ValueError(f"unknown p2t.outcome_adv_norm_mode: {norm_mode!r}")


def compute_p2t_advantage(
    outcome_token_level_rewards: torch.Tensor,
    p2t_token_reward: torch.Tensor,
    process_token_reward: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    config: "AlgoConfig",
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,   # unused; outcome norm is driven by p2t.outcome_adv_norm_mode
) -> tuple[torch.Tensor, torch.Tensor]:
    """Process-to-Token (P2T) advantage as a weighted sum of three components.

        A_{n,i} =   use_outcome * outcome_coef * A^outcome_{n,i}
                  + use_p2t     * p2t_coef     * R^P2T_{n,i}
                  + use_process * process_coef * R^process_{n,i}

    Each term is toggled and weighted independently (config.p2t.*):
        use_outcome_adv (bool), outcome_adv_coef (float), outcome_adv_norm_mode
            (null | group-mean | group-mean-std)   -> A^outcome
        use_p2t_adv (bool),     p2t_adv_coef (float)    -> R^P2T (token reward, Eq.3)
        use_process_adv (bool), process_adv_coef (float)-> R^process (raw step reward, broadcast)

    The paper's Eq.5 is the special case:
        use_outcome=True, outcome_norm="group-mean-std", coef=1;
        use_p2t=True, p2t_coef=1; use_process=False.

    Only the outcome term is group-normalized; the token-level p2t/process terms are
    added directly as (already redistributed) rewards.
    """
    assert config is not None and getattr(config, "p2t", None) is not None, "config.p2t is required for P2T advantage"
    p2t_cfg = config.p2t

    def g(key, default):
        return p2t_cfg.get(key, default) if hasattr(p2t_cfg, "get") else getattr(p2t_cfg, key, default)

    use_outcome = g("use_outcome_adv", True)
    outcome_coef = g("outcome_adv_coef", 1.0)
    outcome_norm = g("outcome_adv_norm_mode", "group-mean-std")

    use_p2t = g("use_p2t_adv", True)
    p2t_coef = g("p2t_adv_coef", 1.0)

    use_process = g("use_process_adv", False)
    process_coef = g("process_adv_coef", 0.0)

    adv = torch.zeros_like(response_mask, dtype=torch.float32)

    if use_outcome and outcome_coef != 0.0:
        a_out = _outcome_advantage(
            outcome_token_level_rewards, response_mask, index, config, epsilon, outcome_norm
        )
        adv = adv + outcome_coef * a_out
    if use_p2t and p2t_coef != 0.0:
        adv = adv + p2t_coef * p2t_token_reward * response_mask
    if use_process and process_coef != 0.0:
        adv = adv + process_coef * process_token_reward * response_mask
...
def compute_reinforce_plus_plus_outcome_advantage(
    ...
    """
    assert config is not None
    gamma = config.gamma    # default = 1.0
    with torch.no_grad():
