...
class RLHFDataset(Dataset):
    ...
    def reset_data_index(self):
        def _set_index(example, idx):
            assert 'extra_info' in example
            extra = example['extra_info']
            if 'index' in extra:
                raw_index = extra['index']
            extra['index_raw'] = raw_index
            extra['index'] = idx
        ...
        self.dataframe = self.dataframe.map(
            _set_index,
            with_indices=True,
            desc="Reset extra_info.index and index_raw",
        )

    # --------------------------------- data_stats ---------------------------------
    def init_data_stats(self):
        print('==> Init data_stats ...')
        self.data_stats = {}   # self.data_stats[data_id][epoch_id] is a dict
        ...
        if epoch != self.current_epoch:
            for i in range(len(self.dataframe)):
                ...
                if data_id not in self.data_stats:
                    self.data_stats[data_id] = [None]
                else:
                    self.data_stats[data_id].append(None)

            self.current_epoch = epoch

        for data_id, info in reorg_batch.items():
            self.data_stats[data_id][-1] = {
                'n_rollout': info['n_rollout'],
                # 'total_reward_list': info['total_reward_list'],
                # 'correctness_reward_list': info['correctness_reward_list'],
                'acc_list': info['acc_list'],
